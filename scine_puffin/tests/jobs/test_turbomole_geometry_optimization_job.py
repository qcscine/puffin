...
class TurbomoleGeometryOptimizationJobTest(JobTestCase):
    ...
    def test_energy(self):
        # import Job
        from scine_puffin.jobs.turbomole_geometry_optimization import TurbomoleGeometryOptimization
        ...
        job = db.Job('turbomole_geometry_optimization')
        calculation = add_calculation(self.manager, model, job, [structure.id()])

        # Run calculation/job
        config = self.get_configuration()
        job = TurbomoleGeometryOptimization()
        ...
        self.assertAlmostEqual(energy.get_data(), -76.27266683034, delta=1e-1)
