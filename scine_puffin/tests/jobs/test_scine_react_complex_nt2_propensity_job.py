...
class ScineReactComplexNt2JobTest(JobTestCase):
    # This test was temporarily disabled because it is numerically unstable.
    # It should be enabled again if numerical stability, and therefore, reliability of the
    # CI can be ensured.
    ...
    def test_propensity_hit(self):
        ...
        # TS + 2 product (1 deduplicated) + 2 spin state changed product (1 deduplicated) + 1 spin changed TS
        assert len(results.structure_ids) == 6
        assert len(results.elementary_step_ids) == 1
        # Total structure count: reactant + TS + 2 product (1 deduplicated)
        # + 2 spin state changed product (1 deduplicated) + 1 spin changed TS
        assert structures.count("{}") == 7
        print(len(results.structure_ids), structures.count("{}"))
        new_elementary_step = db.ElementaryStep(results.elementary_step_ids[-1], elementary_steps)
